...
protected:
	...
	UFUNCTION()
	void TriggerPeculiarityInteraction();
	...
	// 상호작용을 위한 보이지 않는 볼륨 (새로운 루트 컴포넌트)
	UPROPERTY(VisibleAnywhere, BlueprintReadOnly, Category = "Components")
	...
	// 특이사항(Peculiarity) 반복 동작을 위한 타이머 핸들
	FTimerHandle PeculiarityTimerHandle;

	// 에디터에서 혈흔 오버레이 머티리얼 할당 리스트
	UPROPERTY(EditDefaultsOnly, Category = "Anomaly | Config")
	TArray<TObjectPtr<UMaterialInterface>> BloodOverlayMaterialList;
