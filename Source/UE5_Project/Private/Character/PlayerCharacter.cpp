...
void APlayerCharacter::HandleCharging(float DeltaTime)
{
	...
	// '지방'이 이미 가득 찼을 때
	if (GetAttributes() && GetAttributes()->GetTalisman() >= GetAttributes()->GetMaxTalisman())
	{
		Pcc->ShowAutoText(2.0f);
		Pcc->UpdateText(TEXT("지방이 이미 가득 찼습니다."));
		return;
	}

	Pcc->ShowText();
	Pcc->UpdateText(FString::Printf(TEXT("충전 중.. %.1f초"), (RequiredChargeTime - ChargeTime)));
